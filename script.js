...
document.getElementById('confirmHint').addEventListener('click', () => {
    ...
    if (hintValue && !selectedHint.options[selectedHint.selectedIndex].disabled) { // 無効化チェック
        if (confirm(`ヒント: ${hintText}を確認しますか？`)) {
            ...
            switch (hintValue) {
                case '懐中電灯':
                    hintMessage = '「懐中電灯は落ちてたやつなんだけど、中に電池が入ってなかったから、通常の使用目的ではなさそうなんだ」「調べてみたら何かわかるかもしれないな」';
                    break;
                case '包丁':
                    hintMessage = '「包丁はおそらく千田を殺すための凶器で使われたやつなんだがな」「なんか妙なんだよな」「とりあえず、調べてみてくれよ」';
                    break;
                case 'ロープ':
                    hintMessage = '「あぁ、このロープか」「ロープは天井の留め具につながるほうと奈賀岡の首に絡まっている方に千切れていたんだ」「奈賀岡の死因は首元部分の圧迫死だったから、このロープで首を吊って死んだことにほぼ間違いないんだがな」';
                    break;
                case 'タバコ':
                    hintMessage = '「窓際に落ちてたやつだな」「一応補足しておくと、発見されたタバコは人の手によって火が消されたわけじゃなくて、勝手に燃えて火が消えたようなんだ」「おそらくこのタバコを吸ってる最中に殺されたんだろうな」';
                    break;
                case '布団':
                    hintMessage = '「敷きっぱなしになっている布団は、まあこれも当たり前だが奈賀岡のものだ」「千田が来るというのに布団すら畳まないなんてな、まったく……」「話がそれちまったな、布団にはおそらく何もないだろう」';
                    break;
                case '座布団':
                    hintMessage = '「座布団には何もなかったよ...見たらわかるだろう？」「探偵さんの推理力も衰えちまったか？」';
                    break;
                case '雑誌':
                    hintMessage = '「棚に置いてあった雑誌は、奈賀岡が購入したもので間違いないだろう、クローゼットから同一シリーズの雑誌が多数出てきた」「これはおそらく、何ら事件と関係はないだろうね」「探偵さん、しっかりしてくれよな」';
                    break;
                case '卒アル':
                    hintMessage = '「この奈賀岡と千田は君の母校と同じ高校に通っていたようでね、だからいくらか見覚えがあるんじゃあないかな」「爆破予告といい卒業生の死といい、この高校はろくなことがないな」「ああ、ごめん、君のことを言ったわけじゃないんだ　とりあえず、この卒アルの内容は詳しく見てないからわからないが、後で調べてみてもいいと思うぞ」';
                    break;
                default:
